$(function(){
    ...
    $('#calendar').fullCalendar({
        ...
        events: '/SIGEME/CalendarJsonServlet?action=getEventsId',
        // Handle Day Click
    ...
    function modal(data) {
        ...
        if( ! data.event) {
            ...
            var time = data.event.date.split(' ')[1].slice(0, -5);
            //alert(time);
    ...
    $('.modal').on('click', '#add-event',  function(e){
        if(validator(['title', 'description'])) {
            Proxy.addCalendarDTO(new CalendarDTO(0,$('#title').val(),$('#description').val(),
            $('#color').val(),currentDate + ' ' + getTime()),function(result){
                $('.modal').modal('hide');
    ...
    $('.modal').on('click', '#update-event',  function(e){
        if(validator(['title', 'description'])) {
            Proxy.updateCalendarDTO(new CalendarDTO(currentEvent._id,$('#title').val(),$('#description').val(),
            $('#color').val(),currentEvent.date.split(' ')[0]  + ' ' +  getTime()),function(result){
                $('.modal').modal('hide');
                $('#calendar').fullCalendar("refetchEvents");
            });
            /*
    ...
    $('.modal').on('click', '#delete-event',  function(e){
          Proxy.deleteCalendarDTO(new CalendarDTO(currentEvent._id,$('#title').val(),$('#description').val(),
            $('#color').val(),currentEvent.date.split(' ')[0]  + ' ' +  getTime()),function(result){
                $('.modal').modal('hide');
                $('#calendar').fullCalendar("refetchEvents");
            });
